...
const inputClass = 'h-12 rounded-[6px] border border-black/15 bg-white px-4 text-base font-bold text-[#08090a] outline-none transition placeholder:text-black/35 focus:border-[#007780]'
const buttonClass = 'inline-flex h-12 items-center justify-center rounded-[6px] bg-[#007780] px-6 text-sm font-black uppercase tracking-[0.18em] text-white shadow-sm transition hover:-translate-y-0.5 hover:bg-[#006a72] disabled:opacity-60'
...
export function EditableLocalLoginForm() {
  ...
  return (
    <form className="mt-6 grid gap-4" onSubmit={submit}>
      ...
      {message ? <p className={`rounded-[6px] px-4 py-3 text-sm font-bold ${status === 'success' ? 'bg-emerald-50 text-emerald-800' : 'bg-red-50 text-red-700'}`}>{message}</p> : null}
